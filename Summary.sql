...
SELECT url FROM movies WHERE lexemesStarring @@ to_tsquery('harry');
ALTER TABLE movies ADD rank float4;
UPDATE movies SET rank = ts_rank(lexemesSummary,plainto_tsquery((SELECT Summary FROM movies WHERE url='harry-potter-and-the-goblet-of-fire')));
