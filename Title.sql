...
SELECT url FROM movies WHERE lexemesTitle @@ to_tsquery('hitman');
ALTER TABLE movies ADD rank float4;
UPDATE movies SET rank = ts_rank(lexemesTitle,plainto_tsquery((SELECT Title FROM movies WHERE url='the-hitmans-bodyguard')));
