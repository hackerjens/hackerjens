...
SELECT url FROM movies WHERE lexemesSummary @@ to_tsquery('hitman');
ALTER TABLE movies ADD rank float4;
UPDATE movies SET rank = ts_rank(lexemesStarring,plainto_tsquery((SELECT Starring FROM movies WHERE url='the-hitmans-bodyguard')));
